...
<div class="card shadow-sm border-0 mb-4">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-header">
            <h5 class="modal-title">{{ editMode ? "تعديل تشكيل" : "إضافة تشكيل" }}</h5>
          </div>
          ...
          <form @submit.prevent="save">
            ...
            <div class="modal-footer">
              ...
              <button class="btn btn-add">{{ editMode ? "حفظ التعديل" : "إضافة" }}</button>
            </div>
  ...
  const save = async () => {
    if (!editMode.value) await addFormation(form);
    else await updateFormation(form.id, form);
